...
class ShipGameClient(tk.Tk):
    ...
    def player2_turn(self, x, y, btn):
        if self.player1_board[x][y] == "S":
            ...
            self.send_data("HIT", self.player1_board, (x, y))
            if self.check_win():
            ...
            self.player1_board[x][y] = "M"
            self.send_data("MISS", self.player1_board, (x, y))
            self.player_guess_window.destroy()
            self.start_game()
    ...
    def check_win(self):
        ...
        player2_hits = sum(row.count("H") for row in self.player1_board)
        return player2_hits == total_ship_cells
...
if __name__ == "__main__":
    host = 'localhost'
    port = 5000
    client = ShipGameClient(host, port)
    client.mainloop()
